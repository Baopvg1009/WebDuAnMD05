...
const EditCategory = ({ inputs, title }) => {
  ...
  const { discountId } = useParams(); //trùng với tên trong file App.js
  useEffect(() => {
    const getCategoryById = async () => {
      try {
        const docRef = doc(db, "TenKM", discountId);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const docRef = doc(db, "LoaiSP", discountId);
